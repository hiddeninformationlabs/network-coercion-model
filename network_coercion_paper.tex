...
\begin{enumerate}
    ...
    \item We provide counterfactual identification: reshuffling utilities across network positions preserves RNF's predictive power ($\OR = 0.581$, $p < 0.001$), while rewiring networks to matched-density random graphs eliminates the topology gap entirely, confirming that protection is structural rather than compositional. Spatial autocorrelation analysis further demonstrates that coercion patterns are emergent, not mechanical artifacts.
\end{enumerate}
...
We run three conditions at peak regime for each of the 90 topology $\times$ seed combinations ($N = 16{,}938$ resistant agents per condition):

\begin{enumerate}
\item \textbf{Baseline}: Original utility-to-node assignment (production run).
\item \textbf{Reshuffled}: Same network, but utilities randomly permuted across nodes. This preserves both the utility distribution and the network topology exactly while breaking any spatial correlation between preferences and network position.
\item \textbf{Rewired}: Same utilities at same node positions, but the network is rewired to Erd\H{o}s--R\'{e}nyi with matched density. This destroys topology-specific structure (clustering, degree heterogeneity) while preserving the original preference-position mapping.
\end{enumerate}
