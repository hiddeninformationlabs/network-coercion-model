...
\textbf{RNF as a structural mediator.} Resistant neighbor fraction at $t = 0$ is computed from the initial utility draw: agents in neighborhoods where many neighbors drew negative utilities have high RNF. Because the outcome variable (coerced adoption) also requires negative initial utility, RNF and the outcome are mechanically correlated through the utility distribution. This does not invalidate the finding---it remains true that neighborhood composition predicts individual coercion outcomes, and the within-topology regressions confirm this---but RNF should be understood as a structural mediator of topology's effect on coercion, not as a purely exogenous predictor. The causal chain runs from topology to neighborhood composition (RNF) to individual coercion risk.
